...
def run_stage(name: str, cmd: list[str]) -> int:
    ...
    logger.info(
        f"[{name.upper()}] Done in {time.time() - t:.1f}s (exit={r.returncode})"
    )
    return r.returncode
...
def stage_discovery() -> int:
    ...
    codes.append(
        run_stage(
            ...
            [
                sys.executable,
                "discovery/github_pr_outcome_crawler.py",
                "--top-repos",
                "1000",
                "--workers",
                "10",
    ...
    codes.append(
        run_stage(
            ...
            [
                sys.executable,
                "discovery/contributing_md_corpus.py",
                "--min-stars",
                "50",
                "--workers",
                "20",
    ...
    codes.append(
        run_stage(
            "maintainer_interviews",
            [
                sys.executable,
                "discovery/maintainer_interviews.py",
            ],
        )
    )
    return max(codes)
...
def stage_synthesis() -> int:
    ...
    codes.append(
        run_stage(
            "contribution_synthesizer",
            [
                sys.executable,
                "synthesis/contribution_synthesizer.py",
                "--input-dir",
                "data/raw/prs",
                "--output-dir",
                "data/synthesized",
    ...
    codes.append(
        run_stage(
            "maintainer_simulator_train",
            [
                sys.executable,
                "synthesis/maintainer_simulator.py",
                "--train",
                "--data-dir",
                "data/synthesized",
            ],
...
def stage_train() -> int:
    for stage_cmd in [
        ...
        [
            ...
            "--deepspeed",
            "training/configs/ds_config_rl.json",
            "--base_model",
            "checkpoints/sft",
            "--output_dir",
            "checkpoints/grpo",
...
def stage_eval() -> int:
    return run_stage(
        ...
        [
            sys.executable,
            "evaluation/craftbench.py",
            "--model",
            "checkpoints/final",
            "--all",
            "--output",
            "results/craftbench_results.json",
        ],
    )
...
def contribute(repo_url: str, task: str | None) -> None:
    ...
    code_changes = contribution_agent.generate(
        repo_url=repo_url, task=task, conventions=conventions
    )
    ...
    logger.success("Contribution ready:")
